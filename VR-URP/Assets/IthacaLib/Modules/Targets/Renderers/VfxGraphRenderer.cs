...
namespace IthacaLib
{
    public class VfxGraphRenderer : TargetModuleBase<GPUPointCloud>
    {
        ...
        [HideInInspector]
        [SerializeField]
        private bool useSpecificVFXGraph;
        [HideInInspector]
        [SerializeField]
        ...
        public void SetSpecificVisualEffect(VisualEffectAsset effect)
        {
            visualEffectAsset = effect;
            useSpecificVFXGraph = true;
        }
        public VisualEffectAsset GetVisualEffectAsset()
        {
        ...
        public override void HotInit(DataEventArgs<GPUPointCloud> e)
        {
            ...
            if (useSpecificVFXGraph)
            {
                vfxComponent.visualEffectAsset = visualEffectAsset;
            }
            else
            {
        ...
        public override void ProcessData(object sender, DataEventArgs<GPUPointCloud> e)
        {
            ...
            if (IsHotInitComplete)
            {
                binderPC.positions = e.Output.positionMap;  //Note: as the RTs sent by the events are always the sames, it works 
        ...
        void BindExposedProperties(VisualEffectAsset vfxa, GPUPointCloud initPC)
        {
            ...
            if (binderPC == null)
            {
                binderPC = gameObject.GetComponent<PointCloudPositionAndColorBinder>();
                if (binderPC == null)
                {
            ...
            {
                binderAlign = gameObject.GetComponent<PointCloudAlignMatrixBinder>();
                if (binderAlign == null)
                {
    ...
    [VFXBinder("PointCloud/PositionAndColor")]
    public class PointCloudPositionAndColorBinder : VFXBinderBase
    {
        [VFXPropertyBinding("System.Single")]
        public ExposedProperty positionsProperty;

        [VFXPropertyBinding("System.Single")]
        public ExposedProperty colorProperty;

        [VFXPropertyBinding("System.Single")]
        public ExposedProperty pointSizeProperty;
        ...
        public override bool IsValid(VisualEffect component)
        {
            return positions != null
                && colors != null
                && pointSize > 0f
                && component.HasTexture(positionsProperty)
                && component.HasTexture(colorProperty)
             && component.HasFloat(pointSizeProperty);
        }
        ...
        public override void UpdateBinding(VisualEffect component)
        {
            ...
            component.SetFloat(pointSizeProperty, pointSize);
        }
    }

    [VFXBinder("PointCloud/AlignMatrix")]
    public class PointCloudAlignMatrixBinder : VFXBinderBase
    {
        [VFXPropertyBinding("System.Single")]
        public ExposedProperty alignMatProperty;

        public Matrix4x4 alignMat;

        public override bool IsValid(VisualEffect component)
        {
            return alignMat != null
                && component.HasMatrix4x4(alignMatProperty);
        }

        public override void UpdateBinding(VisualEffect component)
        {
            component.SetMatrix4x4(alignMatProperty, alignMat);
        }
    }
}
